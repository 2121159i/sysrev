...
def register(request):
	...
	if request.method == 'POST':
		...
		try:
			...
			if checkUser:
				print 9999999
				return render(request, 'registration/register.html', {'user_form': user_form, 'profile_form': profile_form, 'register_error': True , 'error_type' : 'The username is already taken'})
		...
		if user_form.is_valid() and profile_form.is_valid():
			...
			try:
				...
				if check_email:
					return render(request, 'registration/register.html', {'user_form': user_form, 'profile_form': profile_form, 'register_error': True , 'error_type' : 'The email is already taken'})
			except Exception as v:
				None
			if(password != repeatPassword):
				return render(request, 'registration/register.html', {'user_form': user_form, 'profile_form': profile_form, 'register_error': True , 'error_type' : 'The passwords don\'t match'})
